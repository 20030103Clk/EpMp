  ...
  quality: {
    getQualities: async (params = {}) => {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:136", "GET qualities API response:", response);
        ...
        common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:148", "Get qualities error:", error);
        throw error;
      }
    },
    createQuality: async (data) => {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:163", "Create quality API response:", response);
        if (response && (response[1] || response.data)) {
          if (response[1]) {
            return response[1].data;
          } else if (response.data) {
        ...
        common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:175", "Create quality error:", error);
        throw error;
      }
    },
    deleteQuality: async (id) => {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:186", "Delete quality API response:", response);
        if (response && (response[1] || response.data)) {
          if (response[1]) {
            return response[1].data;
          } else if (response.data) {
        ...
        common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:198", "Delete quality error:", error);
        throw error;
      }
    }
  },
  record: {
    getRecords: async (params = {}) => {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:212", "Get records API response:", response);
        if (response && (response[1] || response.data)) {
          if (response[1]) {
            return response[1].data;
          } else if (response.data) {
        ...
        common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:224", "Get records error:", error);
        throw error;
      }
    }
  }
...
const _sfc_main = {
  ...
  computed: {
    ...
    totalUnqualCount() {
      ...
      common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:365", "总不合格数量计算:", {
        inspectionUnqual,
        productionUnqual,
        unqualQuantity,
  ...
  methods: {
    ...
    // 加载质检记录
    loadQualityRecords() {
      common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:428", "开始加载质检记录...");
      common_vendor.index.showLoading({ title: "加载中..." });
      api.quality.getQualities().then((res) => {
        common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:431", "Get qualities API response:", res);
        common_vendor.index.hideLoading();
        if (res.success) {
          common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:434", "质检记录数据:", res.data.list);
          ...
          common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:445", "inspectionRecords 数组长度:", this.inspectionRecords.length);
        } else {
          common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:447", "加载质检记录失败:", res.message);
          common_vendor.index.showToast({ title: "加载失败", icon: "none" });
        }
      }).catch((error) => {
        common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:451", "加载质检记录失败:", error);
        common_vendor.index.hideLoading();
    ...
    loadProductionRecords() {
      common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:460", "开始加载生产记录...");
      api.record.getRecords({ pageSize: 1e3 }).then((res) => {
        common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:462", "Get records API response:", res);
        if (res.success) {
          common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:464", "生产记录数据:", res.data.list);
          this.productionRecords = res.data.list;
          common_vendor.index.__f__("log", "at pages/production/quality/quality.vue:466", "productionRecords 数组长度:", this.productionRecords.length);
        } else {
          common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:468", "加载生产记录失败:", res.message);
          this.productionRecords = [];
        }
      }).catch((error) => {
        common_vendor.index.__f__("error", "at pages/production/quality/quality.vue:473", "加载生产记录失败:", error);
        this.productionRecords = [];
      });
    },
    openQualityReport() {
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    b: common_vendor.t($options.computedPassRate),
    c: common_vendor.t($options.totalInspections),
    d: common_vendor.t($options.computedPassCount),
    e: common_vendor.t($options.totalUnqualCount),
    f: common_vendor.f($data.inspectionRecords, (item, index, i0) => {
      return common_vendor.e({
    ...
    g: $options.isAdmin,
    h: common_vendor.o((...args) => $options.openQualityReport && $options.openQualityReport(...args), "d5"),
    i: common_vendor.t((/* @__PURE__ */ new Date()).toLocaleDateString("zh-CN")),
    j: common_vendor.t($options.todayInspections),
    k: common_vendor.t($options.rejectCount),
    l: common_vendor.t($options.todayUnqualCount),
    m: common_vendor.t($options.computedTodayPassRate),
    n: common_vendor.f($options.unqualByProduct, (item, k0, i0) => {
      return {
    ...
    o: $options.unqualByProduct.length === 0
  }, $options.unqualByProduct.length === 0 ? {} : {}, {
    p: common_vendor.o((...args) => $options.closeQualityReport && $options.closeQualityReport(...args), "bf"),
    q: common_vendor.sr("qualityReportPopup", "fa3ada11-0")
  });
}
